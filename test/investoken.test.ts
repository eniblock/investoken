...
describe("Investoken tests", function () {
  ...
  it("Should revert upgrade with role ", async function () {
      const Investoken = await ethers.getContractFactory("Investoken");
      const newInvestoken = await Investoken.deploy();
      await newInvestoken.deployed();
  ...
  it("Should revert transfer with pause in beforeTokenTransfer ", async function () {
    await this.investoken.pause();
    await this.investoken.mint(this.admin.address, 100)
    await expect(this.investoken.transfer(this.notAdmin.address, 10)).to.revertedWith("Pausable: paused");
  });

  it("Should revert initialize", async function () {
    await expect(this.investoken.initialize(this.notAdmin.address)).to.revertedWith("Initializable: contract is already initialized");
 });
});
